...
class Kelp(Product):
    def __init__(self):
        super().__init__()
        self.limits = 50
        self.SOFT_LIMIT = 0.6  # TODO ENTRE 0 ET 0.9
        self.MARGIN = 1
...
class Squid(Product):
    ...
    #         return get_mid_price(state, product)
    def take_orders(self, state, product):
        signal = self.olivia.get('OliviaSQUID_INK', None)
        if signal == 'BUY':
            target = self.limits
        elif signal == 'SELL':
            target = -self.limits
        ...
        if diff > 0:
            ...
            self.take_buy_volume = abs(diff)
        elif diff < 0:
            output = go_short(state, -diff, product)
            self.take_sell_volume = abs(diff)
        else:
            output = []
    ...
    def market_orders(self, state: TradingState, product: str):
        if self.olivia.get('OliviaSQUID_INK', None) is not None:
            return []
...
class Croissants(Product):
    ...
    def take_orders(self, state, product):
        ...
        signal = self.olivia.get('OliviaCROISSANTS', None)
        if signal == 'BUY':
            target = self.limits
        elif signal == 'SELL':
            target = -self.limits
...
class Rock(Product):
    ...
    def get_rock_orders(self, state):
        ...
        for product in self.VOUCHER:
            iv = statistics.mean(self.prices['IV_' + product])
            K = self.VOUCHER[product]
            # logger.print(product, DELTA(S, K, T, r, iv)
            #              * get_position(state, product))
    ...
    def hold_orders(self, state, product):
    ...
    def get_orders(self, state):
        ...
        for product in self.VOUCHER:
            ...
            if product in state.order_depths:
                # if product == "VOLCANIC_ROCK_VOUCHER_10500" or product == "VOLCANIC_ROCK_VOUCHER_10250":
                #     orders[product] = self.hold_orders(state, product)
                # else:
    ...
    #     orders['VOLCANIC_ROCK'] = self.get_rock_orders(state)
        ...
        # orders['VOLCANIC_ROCK'] = self.hold_orders(state, 'VOLCANIC_ROCK')
...
class Macarons(Product):
    ...
    # def add_price(self, trader_data, state, product):
    #     # new_price = get_mid_price(state, product)
    #     new_price = state.observations.conversionObservations["MAGNIFICENT_MACARONS"].sunlightIndex
    #     if trader_data is None:
    #         trader_data = [new_price]
    #     else:
    #         trader_data = trader_data + [new_price]
    ...
    def get_signal(self, state):
        if len(self.prices) > 10:
            slope = self.prices[-1] / self.prices[-11] - 1
            projected_sun = self.prices[-1] + slope * self.horizon
            # logger.print(
            #     f"projected_sun: {projected_sun}, slope: {slope}, price: {self.prices[-1]}")
            if projected_sun > self.threshold:
                return "short"
            elif projected_sun < self.threshold:
                return "long"
            else:
                return "neutral"
        else:
    ...
    def take_orders(self, state, product):
        ...
        return []
        output = []
        if self.get_signal(state) == "long":
            target = self.limits
        elif self.get_signal(state) == "short":
            target = -self.limits
        else:
            return []
        diff = target - get_position(state, product)
        # logger.print(
        #     f"target: {target}, position: {get_position(state, product)}, diff: {diff}")
        if diff > 0:
            output = go_long(state, diff, product)
            self.take_buy_volume = abs(diff)
        elif diff < 0:
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        olivia_products = ['CROISSANTS', 'KELP', 'SQUID_INK']
        for product in olivia_products:
            market_trades = state.market_trades.get(product, [])
            own_trades = state.own_trades.get(product, [])
            for trade in market_trades+own_trades:
                if trade.buyer == 'Olivia':
                    traderObject['Olivia'+product] = 'BUY'
                elif trade.seller == 'Olivia':
                    traderObject['Olivia'+product] = 'SELL'
        ...
        for product in self.products_strategy:
            ...
            product_strategy.olivia['OliviaCROISSANTS'] = traderObject.get(
                'OliviaCROISSANTS', None)
            product_strategy.olivia['OliviaKELP'] = traderObject.get(
                'OliviaKELP', None)
            product_strategy.olivia['OliviaSQUID_INK'] = traderObject.get(
                'OliviaSQUID_INK', None)
            ...
            if product == "VOLCANIC_ROCK":
                ...
                orders = market_orders + take_orders
