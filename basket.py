...
class Squid(Product):
    def __init__(self):
        super().__init__()
        self.limits = 50
        self.SOFT_LIMIT = 0.1  # TODO ENTRE 0 ET 0.9
        self.MARGIN = 1
...
#         self.std_deviation_threshold = 2.
...
class Picnic2(Product):
    ...
    def market_orders(self, state: TradingState, product: str):
        ...
        ask = round(fair_value + default_edge)
        if best_ask_above_fair != None:
            if abs(best_ask_above_fair - fair_value) <= 2:
                ask = best_ask_above_fair  # join
            else:
                ask = best_ask_above_fair - 1  # penny
        ...
        bid = round(fair_value - default_edge)
        if best_bid_below_fair != None:
            if abs(fair_value - best_bid_below_fair) <= 2:
                bid = best_bid_below_fair
            else:
                bid = best_bid_below_fair + 1
...
class Spread1(Product):
    ...
    def get_arb_orders(self, state, product: str):
        ...
        target = self.get_arb_signal(state, product)[product]

        diff = target - get_position(state, product)
        if diff > 0:
            return self.go_long(state, diff, product)
        elif diff < 0:
            return self.go_short(state, -diff, product)
        else:
            return []
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths.keys():
            ...
            spread = Spread1()
            result[product] = spread.get_arb_orders(state, product)
